def repeating_to_single_string_converter(multi_occurring_string):
    ...
    for index_for_multi_occurrence in multi_occurring_string:
        if index_for_multi_occurrence.lower() not in \
                string_after_single_occurrence.lower():
            string_after_single_occurrence = \
                string_after_single_occurrence \
                + index_for_multi_occurrence
...
def is_repeating(repeating_character_string, key_character):
    ...
    for index_for_repeating_character in repeating_character_string:
        counter = counter + 1 \
            if index_for_repeating_character.lower() == key_character.\
            lower() else counter
...
def remove_given_character(input_string, key_character):
    character_removal_index = input_string.find(key_character)

    if character_removal_index == 0:
        input_string = input_string[1:]
    elif character_removal_index == len(input_string) - 1:
        input_string = input_string[0: len(input_string) - 1]
    else:
        input_string = input_string[0:character_removal_index] \
                       + input_string[character_removal_index + 1:
                                      len(input_string)]

    return input_string


def remove_character_first_occurance(removing_string):
    ...
    # keeeps tab of removed characters
    removed_list = []
    ...
    for index_of_removing_string in removing_string:
        if is_repeating(removing_string, index_of_removing_string) and \
            ...
            string_after_removal = \
                remove_given_character(removing_string,
                                       index_of_removing_string)
